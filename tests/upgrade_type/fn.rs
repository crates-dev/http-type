...
fn test_upgrade_type_websocket() {
    ...
    assert!(!upgrade.is_h2());
    assert!(!upgrade.is_h2c());
    assert!(!upgrade.is_h3());
...
#[test]
fn test_upgrade_type_h2() {
    let upgrade: UpgradeType = UpgradeType::H2;
    assert!(!upgrade.is_ws());
    assert!(upgrade.is_h2());
    assert!(!upgrade.is_h2c());
    assert!(!upgrade.is_h3());
    assert!(!upgrade.is_tls());
    assert!(!upgrade.is_unknown());
    assert_eq!(upgrade.to_string(), "h2");
}
...
#[test]
fn test_upgrade_type_h3() {
    let upgrade: UpgradeType = UpgradeType::H3;
    assert!(!upgrade.is_ws());
    assert!(!upgrade.is_h2());
    assert!(!upgrade.is_h2c());
    assert!(upgrade.is_h3());
    assert!(!upgrade.is_tls());
    assert!(!upgrade.is_unknown());
    assert_eq!(upgrade.to_string(), "h3");
}
...
#[test]
fn test_upgrade_type_from_str_h2() {
    let upgrade: UpgradeType = "h2".parse().unwrap();
    assert_eq!(upgrade, UpgradeType::H2);
}
...
#[test]
fn test_upgrade_type_from_str_h3() {
    let upgrade: UpgradeType = "h3".parse().unwrap();
    assert_eq!(upgrade, UpgradeType::H3);
}
...
fn test_upgrade_type_from_str_case_insensitive() {
    ...
    let upgrade4: UpgradeType = "H2".parse().unwrap();
    let upgrade5: UpgradeType = "H3".parse().unwrap();
    assert_eq!(upgrade1, UpgradeType::WebSocket);
    assert_eq!(upgrade2, UpgradeType::WebSocket);
    assert_eq!(upgrade3, UpgradeType::H2c);
    assert_eq!(upgrade4, UpgradeType::H2);
    assert_eq!(upgrade5, UpgradeType::H3);
}
...
fn test_upgrade_type_eq() {
    ...
    assert_ne!(UpgradeType::WebSocket, UpgradeType::H2);
    assert_ne!(UpgradeType::H2, UpgradeType::H2c);
    assert_ne!(UpgradeType::H2c, UpgradeType::H3);
    assert_eq!(
